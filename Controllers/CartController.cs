...
public class CartController : Controller
{
    ...
    public async Task<IActionResult> Checkout()
    {
        ...
        var order = new Order
        {
            ...
            Status = "Покупка успешно оформлена (демо режим)",
        ...
        foreach (var item in fullCart.Items)
        {
        ...
        TempData["Success"] = "Покупка успешно оформлена (демо режим)";
        return RedirectToAction("My", "Orders");
    }

    private async Task<Cart> GetOrCreateCartAsync()
    {
        var userId = _userManager.GetUserId(User) ?? throw new InvalidOperationException();
